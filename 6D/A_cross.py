...
def encryption(image, seqs):
    ...
    for i in range(1, w):
        for j in range(2 * i):
            col = int(j + h/3 - i)
            if 0 <= col < h and m < seq_length:
                image[i][col] ^= seqs[0][m]
                m += 1
...
def main_Cross(g, b, r, mode, seqs):
    w = r.shape[0]
    # Create a single image by concatenating the three channels
    image = np.concatenate((g, b, r), axis=1)

    if mode == 'encryption':
        image = encryption(image, seqs)
    elif mode == 'decryption':
        image = decryption(image, seqs)
...
seqs = np.ones((2, 16), dtype=np.uint8)  # Using uint8 for image operations
g = np.zeros((4, 4), dtype=np.uint8)
b = np.zeros((4, 4), dtype=np.uint8)
r = np.zeros((4, 4), dtype=np.uint8)
g,b,r = main_Cross(g, b, r, 'encryption', seqs)
g,b,r = main_Cross(g, b, r, 'decryption', seqs)
